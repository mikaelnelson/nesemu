...
class PpuPalette {
 public:
  ...
  static uint32_t get_color(uint8_t palette_index) {
    if (palette_index >= PALETTE_TABLE_CNT) {
      spdlog::error("Palette index out of range: {}", palette_index);
      return 0;
    }
    return _palette_table[palette_index] | 0xFF000000;
  }

 private:
  static constexpr int PALETTE_TABLE_CNT = 64;

  // http://www.romdetectives.com/Wiki/index.php?title=NES_Palette
  static constexpr std::array<uint32_t, PALETTE_TABLE_CNT> _palette_table = {
  ...
  static_assert(_palette_table.size() == PALETTE_TABLE_CNT);
};
