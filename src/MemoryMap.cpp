...
bool MemoryMap::register_device(const std::shared_ptr<IMemory> &device,
    ...
    if (!result) {
        spdlog::warn("Device (0x{0:X} - 0x{1:X}) overlaps with existing "
                     "device, cannot "
                     "register",
...
std::optional<MemoryMap::MapEntry>
MemoryMap::find_entry(const uint16_t address) const {
...
bool MemoryMap::add_entry(const MapEntry &new_entry) {
    ...
    for (const auto &map_entry: _address_map) {
        ...
        if (new_entry_start >= map_entry_start &&
